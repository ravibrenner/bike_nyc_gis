...
# Community districts
```{r}
nyc_cd <- soc_read("https://data.cityofnewyork.us/City-Government/Community-Districts/5crt-au7u/about_data")

nyc_cd |>
  mutate(boro = str_sub(as.character(boro_cd),1,1),
         boro_name = case_match(boro,
                                ...
                                "5" ~ "Staten Island")) |>
  select(-starts_with(":")) |>
  st_write("data/nyc_cd.shp",
           delete_layer = TRUE)
```

# ZIP codes
```{r}
nyc_zips <- soc_read("https://data.cityofnewyork.us/Health/Modified-Zip-Code-Tabulation-Areas-MODZCTA-/pri4-ifjk/about_data")

nyc_zips |>
  mutate(zip = modzcta) |>
  rename(geometry = the_geom) |>
  select(zip, geometry, pop_est, -starts_with(":")) |>
  st_write("data/nyc_zips.shp")
```
